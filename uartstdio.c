...
static volatile uint32_t uarttxwriteindex = 0;
static volatile uint32_t uarttxreadindex = 0;

static unsigned char uartrxbuffer[UART_RX_BUFFER_SIZE];
static volatile uint32_t uartrxreadindex = 0;
static volatile uint32_t uartrxwriteindex = 0;
...
static uint32_t g_uartbase = 0;
static const char * const g_hex = "0123456789abcdef";
static const uint32_t g_uartbaselist[3] = {
    UART0_BASE, UART1_BASE, UART2_BASE
};
static const uint32_t g_uartintlist[3] = {
    INT_UART0, INT_UART1, INT_UART2
};
static uint32_t g_portnum;
static const uint32_t g_uartperiphlist[3] = {
    SYSCTL_PERIPH_UART0, SYSCTL_PERIPH_UART1, SYSCTL_PERIPH_UART2
};

static bool uartstdio_isbufferfull(volatile uint32_t *read_ptr, volatile uint32_t *write_ptr, uint32_t size) {
    uint32_t write;
    uint32_t read;
...
static bool uartstdio_isbufferempty(volatile uint32_t *read_ptr, volatile uint32_t *write_ptr) {
    uint32_t write;
    uint32_t read;
...
static uint32_t uartstdio_getbuffercount(volatile uint32_t *read_ptr, volatile uint32_t *write_ptr, uint32_t size) {
    uint32_t write;
    uint32_t read;
...
static void uartstdio_primetransmit(uint32_t base) {
...
void uartstdio_config(uint32_t portnum, uint32_t baud, uint32_t clockfreq) {
    //
    // Check the arguments.
    //
...
int uartstdio_write(const char *buf, uint32_t len) {
    uint32_t i;
...
int uartstdio_gets(char *buf, uint32_t len) {
    uint32_t i = 0;
    int8_t character;
...
void uartstdio_vprintf(const char *str_ptr, va_list vaArgP) {
    uint32_t i, value, position, count, base, negative;
...
convert:
                ...
                case 'f':
                    fpval = va_arg(vaArgP, double);
                    position = ftoa(fpval, fpbuffer);
                    
                    // Write the string.
                    uartstdio_write(fpbuffer, position);
                    uartstdio_write(" - ", 3);
                    uartstdio_write("0123456789", position);
                    break;
...
int uartstdio_rxbytesavail(void) {
    return(RX_BUFFER_USED);
}
int uartstdio_txbytesfree(void) {
    return(TX_BUFFER_FREE);
}

int uartstdio_peek(unsigned char character) {
    int count;
    int available;
    uint32_t readindex;
